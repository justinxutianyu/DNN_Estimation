...
def compute(city, optimizer_flag):
	######################## set parameters ##################
	...
	distance_matrix, test_distance_matrix, max_distance = util.load_landmark_data(
	    city, path)
...
def dif_input_compute(city, optimizer_flag, input_flag):
	######################## set parameters ##################
	timestr = time.strftime("%Y%m%d-%H%M%S")
	...
	filename = input_flag +"_"+ city.name(timestr)

	########################  loading data and graph #######################
	path = "/mnt/Project/data"
	distance_matrix, test_distance_matrix, max_distance = util.load_input_data(
	    city, path, input_flag)
...
def dif_layer_compute(city, optimizer_flag, layer_num):
	...
	unit = city.unit
	output_size = 1
	x = tf.placeholder(tf.float32, [None, input_size], name='x')  # inpute features
	y_ = tf.placeholder(tf.float32, [None, output_size], name='y')  # predictions
	...
	if layer_num == 3:
		...
		y  = layer(h2, unit, output_size, 'sigmoid')
	if layer_num == 4:
		h1 = layer(x, input_size, unit, 'relu')
		h2 = layer(h1, unit, unit, 'relu')
	...
	if layer_num == 5:
		...
		h2 = layer(h1, unit, unit, 'relu')
		h3 = layer(h2, unit, unit, 'relu')
		h4 = layer(h3, unit, unit, 'relu')
		y  = layer(h4, unit, output_size, 'sigmoid')
	if layer_num == 6:
		h1 = layer(x, input_size, unit, 'relu')
		h2 = layer(h1, unit, unit, 'relu')
		h3 = layer(h2, unit, unit, 'relu')
		h4 = layer(h3, unit, unit, 'relu')
		h5 = layer(h4, unit, unit, 'relu')
		y  = layer(h5, unit, output_size, 'sigmoid')
