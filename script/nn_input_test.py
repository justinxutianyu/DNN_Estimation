...
def input_test(city):
	...
	flag = 'adj'
	adj_loss_array, adj_rel_error, adj_abs_error = compute.dif_input_compute(city, "Adam", flag)
	loss_array_list.append(adj_loss_array)
	rel_error_list.append(adj_rel_error)
	abs_error_list.append(adj_abs_error)

	flag = 'all'
	all_loss_array, all_rel_error, all_abs_error = compute.dif_input_compute(city, "Adam", flag)
	loss_array_list.append(all_loss_array)
	rel_error_list.append(all_rel_error)
	abs_error_list.append(all_abs_error)

	flag = 'landmark'
	landmark_loss_array, landmark_rel_error, landmark_abs_error = compute.dif_input_compute(city, "Adam", flag)
	...
	plt.savefig("picture/"+city.location+"_"+optimize_time+"_input_loss_test.png")
	plt.clf()

	for i in range(3):
		plt.plot(rel_error_list[i])
	plt.legend(labels, loc='upper right')
	plt.savefig("picture/"+city.location+"_"+optimize_time+"_input_relerror_test.png")
	plt.clf()
